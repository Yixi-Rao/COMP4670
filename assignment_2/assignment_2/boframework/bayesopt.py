...
class BO:
    ...
    def __call__(self, acquisition: Callable, xi: float, n_iter: int, name: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(n_iter):
            ...
            y_next         = _f(X_next)
            self._X_sample = np.row_stack((self._X_sample, X_next)) 
            self._Y_sample = np.row_stack((self._Y_sample, y_next)) 
            
            # NOTE scheduler design
            # The basic idea of xi scheduler is using a high xi 
            # in the early stage, and using small xi to fine-turned in the final stage.
            # The xi scheduler I used is the exponential decay scheduler, which is xi = xi_0 * (0.97)^i, where xi_0 = the initial xi
            # so the xi decreases exponentially over i. Unlike the linear scheduler, The characteristic of exponential decay is that it decreases rapidly when i < 5,
            # and decreases smoothly and relatively slowly when i > 10.
            # so it will have high probability to converge and find the optimum
            xi = xi * 0.97 
            # DO NOT CHANGE
