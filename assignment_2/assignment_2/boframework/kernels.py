...
class Matern(Kernel):
    ...
    def __call__(self, X: np.ndarray, Y: np.ndarray = None) -> np.ndarray:
        ...
        if self.nu == 1.5:
            ...
            return (self.variance) * (1 + (np.sqrt(3) / length_scale) * D_XY) * (np.exp(-(np.sqrt(3) / length_scale) * D_XY))
        elif self.nu == 2.5:
            if Y is None:
                Y = X
            else:
            ...
            return (self.variance) * (1 + ((np.sqrt(5) / length_scale) + (5 / (3 * (length_scale) ** 2)) * D_XY) * D_XY) * (np.exp(-(np.sqrt(5) / length_scale) * D_XY))
        else:
            # Do not change
